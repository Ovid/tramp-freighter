...
class NodeMaterial extends ShaderMaterial {
  ...
  construct(builder) {
    ...
    fragmentStack.outputNode = this.constructOutput(
      builder,
      fragmentStack,
      outgoingLightNode,
      diffuseColor.a
  ...
  constructLighting(builder) {
    ...
    const lightingModelNode = lightsNode
      ? this.constructLightingModel(builder)
      : null;
    ...
    if (
      (this.emissiveNode && this.emissiveNode.isNode === true) ||
      (material.emissive && material.emissive.isColor === true)
    ) {
      outgoingLightNode = outgoingLightNode.add(
        this.emissiveNode ? vec3(this.emissiveNode) : materialEmissive
      );
    }
  ...
  constructOutput(builder, stack, outgoingLight, opacity) {
    ...
    if (!toneMappingNode && renderer.toneMapping !== NoToneMapping) {
      toneMappingNode = toneMapping(
        renderer.toneMapping,
        reference('toneMappingExposure', 'float', renderer),
        outgoingLight
      );
    }
