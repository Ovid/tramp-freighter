...
export const BurnNode = new ShaderNode(({ base, blend }) => {
  const fn = (c) =>
    blend[c]
      .lessThan(EPSILON)
      .cond(blend[c], base[c].invert().div(blend[c]).invert().max(0));
...
export const DodgeNode = new ShaderNode(({ base, blend }) => {
  const fn = (c) =>
    blend[c].equal(1.0).cond(blend[c], base[c].div(blend[c].invert()).max(0));
...
export const OverlayNode = new ShaderNode(({ base, blend }) => {
  const fn = (c) =>
    base[c]
      .lessThan(0.5)
      .cond(
        base[c].mul(blend[c], 2.0),
        base[c].invert().mul(blend[c].invert()).invert()
      );
