...
const RE_IndirectSpecular_Physical = new ShaderNode((inputs) => {
  ...
  const diffuse = diffuseColor.mul(
    singleScattering.add(multiScattering).invert()
  );
...
const RE_IndirectDiffuse_Physical = new ShaderNode((inputs) => {
  ...
  reflectedLight.indirectDiffuse.addAssign(
    irradiance.mul(BRDF_Lambert.call({ diffuseColor }))
  );
});
...
const RE_Direct_Physical = new ShaderNode((inputs) => {
  ...
  reflectedLight.directDiffuse.addAssign(
    irradiance.mul(BRDF_Lambert.call({ diffuseColor: diffuseColor.rgb }))
  );

  reflectedLight.directSpecular.addAssign(
    irradiance.mul(
      BRDF_GGX.call({ lightDirection, f0: specularColor, f90: 1, roughness })
    )
  );
});

const RE_AmbientOcclusion_Physical = new ShaderNode(
  ({ ambientOcclusion, reflectedLight }) => {
...
const physicalLightingModel = lightingModel(
  RE_Direct_Physical,
  RE_IndirectDiffuse_Physical,
  RE_IndirectSpecular_Physical,
  RE_AmbientOcclusion_Physical
);
