...
const MtlXElements = [
  ...
  new MtlXElement('noise2d', mx_noise_float, [
    'texcoord',
    'amplitude',
    'pivot',
  ]),
  new MtlXElement('noise3d', mx_noise_float, [
    'texcoord',
    'amplitude',
    'pivot',
  ]),
  ...
  new MtlXElement('worleynoise2d', mx_worley_noise_float, [
    'texcoord',
    'jitter',
  ]),
  new MtlXElement('worleynoise3d', mx_worley_noise_float, [
    'texcoord',
    'jitter',
  ]),
...
class MaterialXNode {
  ...
  get referencePath() {
    ...
    if (this.nodeGraph !== null && this.output !== null) {
      ...
      referencePath =
        this.getNodeGraph().nodePath +
        '/' +
        (this.nodeName || this.interfaceName);
    }
  ...
  getNode() {
    ...
    if (this.isConst) {
      ...
      if (element === 'convert') {
        ...
        const uvTiling = mx_transform_uv(
          ...this.getNodesByNames(['uvtiling', 'uvoffset'])
        );
        ...
        node = nodeElement.nodeFunc(
          ...this.getNodesByNames(...nodeElement.params)
        );
      }
    }
    ...
    if (node === null) {
      console.warn(
        `THREE.MaterialXLoader: Unexpected node ${new XMLSerializer().serializeToString(this.nodeXML)}.`
      );
  ...
  setStandardSurfaceToGltfPBR(material) {
    ...
    if (inputs.base && inputs.base_color)
      colorNode = mul(inputs.base, inputs.base_color);
