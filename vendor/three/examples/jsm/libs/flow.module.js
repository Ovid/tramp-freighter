...
const removeDOMClass = (dom, classList) => {
  if (classList)
    classList
      .split(' ')
      .forEach((alignClass) => dom.classList.remove(alignClass));
};

const addDOMClass = (dom, classList) => {
  if (classList)
    classList.split(' ').forEach((alignClass) => dom.classList.add(alignClass));
};
...
class Element extends Serializer {
  ...
  connect(element = null) {
    ...
    if (element !== null) {
      ...
      if (
        dispatchEventList(this.events.valid, this, element, 'connect') === false
      ) {
        return false;
      }
  ...
  _createIO(type) {
    ...
    const onConnectEvent = (e) => {
      ...
      const link =
        type === defaultOutput ? new Link(this) : new Link(null, this);
      ...
      draggableDOM(
        ...
        (data) => {
          ...
          const isInvalid =
            previewLink.inputElement !== null &&
            previewLink.outputElement !== null &&
            previewLink.inputElement.inputLength > 0 &&
            previewLink.outputElement.outputLength > 0 &&
            dispatchEventList(
              previewLink.inputElement.events.valid,
              previewLink.inputElement,
              previewLink.outputElement,
              data.dragging ? 'dragging' : 'dragged'
            ) === false;
          ...
          if (!data.dragging) {
            ...
            if (selected !== null && !isInvalid) {
              ...
              if (
                link.inputElement.inputLength > 0 &&
...
class Canvas extends Serializer {
  constructor() {
    ...
    const onTouchZoom = (e) => {
      if (e.touches && e.touches.length === 2) {
        ...
        const delta = (touchData.distance - distance) * 0.003;
    ...
    const onTouchMove = (e) => {
      if (e.touches && e.touches.length === 1) {
        ...
        this.scrollLeft =
          touchData.scrollLeft + (clientX - touchData.clientX) / zoom;
        this.scrollTop =
          touchData.scrollTop + (clientY - touchData.clientY) / zoom;
      }
    };
      ...
      (data) => {
        ...
        if (data.scrollLeft === undefined) {
          ...
          const overMapScreen =
            clientMapX > screen.x &&
            clientMapY > screen.y &&
            clientMapX < screen.x + screen.width &&
            clientMapY < screen.y + screen.height;
          ...
          if (overMapScreen === false) {
            ...
            let scrollTop =
              -this._mapInfo.top -
              clientMapY * (this._mapInfo.height / this.mapCanvas.height);
  ...
  updateMozTransform() {
    ...
    this.contentDOM.style['-moz-transform-origin'] =
      '-' + this.contentDOM.style.left + ' -' + this.contentDOM.style.top;
  }
  ...
  updateMap() {
    ...
    const mapScale =
      Math.min(mapCanvas.width / boundsWidth, mapCanvas.height / boundsHeight) *
      0.5;
    ...
    for (const link of links) {
      ...
      if (draggingLink || length === 1) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          drawLine(
            aPos.x * zoom,
            (aPos.y + aPosY - aCenterY) * zoom,
            bPos.x * zoom,
            (bPos.y + bPosY - bCenterY) * zoom,
            false,
            2,
            colorA,
            drawContext,
...
class ContextMenu extends Menu {
  ...
  setTarget(target = null) {
    if (target !== null) {
      const onContextMenu = (e) => {
        ...
        if (e.pointerType !== 'mouse' || (e.pageX === 0 && e.pageY === 0))
          return;
  ...
  show() {
    ...
    let offsetX = Math.min(
      window.innerWidth - (domRect.x + domRect.width + 10),
      0
    );
    let offsetY = Math.min(
      window.innerHeight - (domRect.y + domRect.height + 10),
      0
    );
    ...
    if (this.opened) {
      ...
      if (offsetY < 0)
        this.dom.style.transform = 'translateY( calc( 32px - 100% ) )';
    }
      ...
      if (key === 'ArrowUp') {
        ...
        if (this.forceAutoComplete) {
          ...
          if (this.filteredIndex === null)
            this.filteredIndex = this.filtered.length - 1;
        } else {
