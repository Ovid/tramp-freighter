...
describe('Complete Game Flow Integration Tests', () => {
    ...
    describe('New Game → Jump → Trade → Refuel → Save → Load Cycle', () => {
        it('should complete a full game cycle successfully', async () => {
            ...
            expect(gameStateManager.state.player.credits).toBe(creditsAfterSale - (orePrice * buyQuantity));
            ...
            expect(gameStateManager.state.ship.fuel).toBe(fuelAfterJump + refuelAmount);
            expect(gameStateManager.state.player.credits).toBe(creditsAfterPurchase - (fuelPrice * refuelAmount));
            ...
            const stateBeforeLoad = JSON.parse(JSON.stringify(gameStateManager.state));
            ...
            expect(loadedState.player.currentSystem).toBe(stateBeforeLoad.player.currentSystem);
            expect(loadedState.player.daysElapsed).toBe(stateBeforeLoad.player.daysElapsed);
            expect(loadedState.ship.fuel).toBe(stateBeforeLoad.ship.fuel);
            expect(loadedState.ship.cargo.length).toBe(stateBeforeLoad.ship.cargo.length);
            expect(loadedState.ship.cargo[0].good).toBe(stateBeforeLoad.ship.cargo[0].good);
            expect(loadedState.ship.cargo[0].qty).toBe(stateBeforeLoad.ship.cargo[0].qty);
        });
    ...
    describe('Error Scenario Integration Tests', () => {
        ...
        it('should handle refuel beyond capacity', () => {
            ...
            expect(refuelResult.success).toBe(false);
            expect(refuelResult.reason).toContain('Cannot refuel beyond 100% capacity');
    ...
    describe('UI Integration Tests', () => {
        ...
        it('should update HUD location when jumping', () => {
            ...
            expect(document.getElementById('hud-distance').textContent).toBe('0.0 LY');
            
            // Jump to Alpha Centauri
            navigationSystem.executeJump(gameStateManager, 1);
            
            expect(document.getElementById('hud-system').textContent).toBe('Alpha Centauri A');
            expect(document.getElementById('hud-distance').textContent).not.toBe('0.0 LY');
        });
    });
