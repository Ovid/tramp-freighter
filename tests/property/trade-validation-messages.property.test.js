...
describe.skip('Trade Validation Messages', () => {
  ...
  it('should show error message when insufficient credits', () => {
    ...
    const system = TEST_STAR_DATA.find(
  ...
  it('should show appropriate message when both constraints fail', () => {
    ...
    const hasError =
      validationMessage.textContent.includes('Cargo capacity full') ||
  ...
  it('should show different messages for different goods based on price', () => {
    ...
    const electronicsPrice = TradingSystem.calculatePrice(
      'electronics',
      system.type
    );
    ...
    const electronicsItem = uiManager.createGoodItem(
      'electronics',
      electronicsPrice
    );

    const grainMessage = grainItem.querySelector('.validation-message');
    const electronicsMessage = electronicsItem.querySelector(
      '.validation-message'
    );
