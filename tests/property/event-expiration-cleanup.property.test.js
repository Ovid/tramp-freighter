...
describe('Property: Event Expiration Cleanup', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (activeEvents, currentDay) => {
          ...
          const filteredEvents = EconomicEventsSystem.removeExpiredEvents(
            activeEvents,
            currentDay
          );
          ...
          const expiredCount = activeEvents.filter(
          ...
          const nonExpiredCount = activeEvents.filter(
            (e) => e.endDay >= currentDay
          ).length;
  ...
  it('should handle empty active events array', () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 1000 }), (currentDay) => {
    ...
    fc.assert(
      fc.property(
        ...
        (currentDay) => {
          ...
          const result = EconomicEventsSystem.removeExpiredEvents(
            activeEvents,
            currentDay
          );
  ...
  it('should handle all events active', () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 900 }), (currentDay) => {
        const activeEvents = [
          {
            id: 'event1',
            type: 'mining_strike',
