...
describe('Price Knowledge - Staleness Increment (Property Tests)', () => {
  it('Property 9: For any time advancement, the lastVisit counter for all systems in the price knowledge database should increment by the number of days passed', () => {
    ...
    fc.assert(
      fc.property(
        visitedSystemsGenerator,
        daysGenerator,
        (visitedSystems, daysToAdvance) => {
          ...
          for (const systemId in priceKnowledgeBefore) {
            initialLastVisits[systemId] =
              priceKnowledgeBefore[systemId].lastVisit;
          }
          ...
          for (const systemId in priceKnowledgeAfter) {
            const expectedLastVisit =
              initialLastVisits[systemId] + daysToAdvance;
            expect(priceKnowledgeAfter[systemId].lastVisit).toBe(
              expectedLastVisit
            );
          }
        }
  ...
  it('Property 9 (variant): Multiple time advancements should accumulate staleness', () => {
    ...
    const advancementsGenerator = fc.array(fc.integer({ min: 1, max: 20 }), {
      minLength: 2,
      maxLength: 5,
    });
      ...
      fc.property(
        ...
        (visitedSystems, daysToAdvance) => {
          ...
          for (let i = 1; i < visitedSystems.length; i++) {
            expect(priceKnowledge[visitedSystems[i]].lastVisit).toBe(
              daysToAdvance
            );
          }
        }
