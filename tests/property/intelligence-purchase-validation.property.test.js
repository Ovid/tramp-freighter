        ...
        fc.assert(
            fc.property(
                ...
                (cost, credits) => {
                    ...
                    const validation = InformationBroker.validatePurchase(actualCost, credits);
                    
                    // Should be invalid
                    expect(validation.valid).toBe(false);
                    expect(validation.reason).toBe('Insufficient credits for intelligence');
                }
            ),
            { numRuns: 100 }
        );
    ...
    it('should validate correctly for all intelligence price tiers', () => {
        fc.assert(
            fc.property(
                ...
                // Pick a price tier
                fc.constantFrom(PRICES.RECENT_VISIT, PRICES.NEVER_VISITED, PRICES.STALE_VISIT),
                (credits, cost) => {
                    const validation = InformationBroker.validatePurchase(cost, credits);
