...
describe('Property 9: Mechanical Failure Thresholds', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (hullCondition, rng) => {
          ...
          const failure = gameStateManager.dangerManager.checkMechanicalFailure(gameState, rng);
          ...
          if (hullCondition < FAILURE_CONFIG.HULL_BREACH.CONDITION_THRESHOLD) {
            ...
            if (rng < FAILURE_CONFIG.HULL_BREACH.CHANCE) {
              expect(failure).toEqual({ type: 'hull_breach', severity: hullCondition });
            } else {
              // If hull breach doesn't occur, other failures might still happen
              expect(failure === null || failure.type !== 'hull_breach').toBe(true);
            }
          } else {
    ...
    fc.assert(
      fc.property(
        ...
        (engineCondition, rng) => {
          ...
          if (engineCondition < FAILURE_CONFIG.ENGINE_FAILURE.CONDITION_THRESHOLD) {
            // Below threshold: engine failure should be possible
            if (rng < FAILURE_CONFIG.ENGINE_FAILURE.CHANCE) {
              expect(failure).toEqual({ type: 'engine_failure', severity: engineCondition });
            } else {
              // If engine failure doesn't occur, other failures might still happen
              expect(failure === null || failure.type !== 'engine_failure').toBe(true);
            }
          } else {
    ...
    fc.assert(
      fc.property(
        ...
        (lifeSupportCondition, rng) => {
          ...
          if (lifeSupportCondition < FAILURE_CONFIG.LIFE_SUPPORT.CONDITION_THRESHOLD) {
            // Below threshold: life support failure should be possible
            if (rng < FAILURE_CONFIG.LIFE_SUPPORT.CHANCE) {
              expect(failure).toEqual({ type: 'life_support', severity: lifeSupportCondition });
            } else {
            ...
            expect(failure === null || failure.type !== 'life_support').toBe(true);
          }
        }
      ),
      { numRuns: 100 }
    ...
    fc.assert(
      fc.property(
        ...
        (rng) => {
          ...
          const gameState = gameStateManager.getState();
          gameState.ship.hull = FAILURE_CONFIG.HULL_BREACH.CONDITION_THRESHOLD - 1;
          gameState.ship.engine = FAILURE_CONFIG.ENGINE_FAILURE.CONDITION_THRESHOLD - 1;
          gameState.ship.lifeSupport = FAILURE_CONFIG.LIFE_SUPPORT.CONDITION_THRESHOLD - 1;
  ...
  it('should return null when all systems are above failure thresholds', () => {
    fc.assert(
      fc.property(
        fc.integer({ min: FAILURE_CONFIG.HULL_BREACH.CONDITION_THRESHOLD, max: 100 }), // hull above threshold
        fc.integer({ min: FAILURE_CONFIG.ENGINE_FAILURE.CONDITION_THRESHOLD, max: 100 }), // engine above threshold
        fc.integer({ min: FAILURE_CONFIG.LIFE_SUPPORT.CONDITION_THRESHOLD, max: 100 }), // life support above threshold
        fc.float({ min: 0, max: 1 }), // random number
