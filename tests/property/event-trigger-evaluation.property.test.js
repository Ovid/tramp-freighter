            ...
            fc.property(
                ...
                fc.array(
                    fc.record({
                        ...
                        type: fc.constantFrom('G5', 'K2', 'M3', 'A1', 'F7', 'O9', 'B5', 'L4', 'T6', 'D1'),
                ...
                ).map((systems, index) => systems.map((system, i) => ({ ...system, id: i }))),
                (gameState, starData) => {
                    // Update events for the current day
                    const updatedEvents = EconomicEventsSystem.updateEvents(gameState, starData);
        ...
        fc.assert(
            fc.property(
                ...
                (system) => {
                    ...
                    expect(EconomicEventsSystem.isSystemEligible(system, miningStrike)).toBe(isMiningSystem);
                    
                    // Test medical_emergency eligibility (any system)
                    const medicalEmergency = EconomicEventsSystem.EVENT_TYPES.medical_emergency;
                    expect(EconomicEventsSystem.isSystemEligible(system, medicalEmergency)).toBe(true);
                    ...
                    expect(EconomicEventsSystem.isSystemEligible(system, festival)).toBe(isCoreSystem);
                    
                    // Test supply_glut eligibility (any system)
                    const supplyGlut = EconomicEventsSystem.EVENT_TYPES.supply_glut;
                    expect(EconomicEventsSystem.isSystemEligible(system, supplyGlut)).toBe(true);
                }
            ),
            { numRuns: 100 }
        );
