    ...
    fc.assert(
      fc.property(
        ...
        (originData, destData) => {
          ...
          const result = animationSystem.calculateSideViewPosition(
            originPos,
            destPos,
            distance
          );
          ...
          if (expectedBaseDistance >= ANIMATION_CONFIG.MIN_SIDE_VIEW_DISTANCE) {
            ...
            expect(cameraDistance).toBeCloseTo(
          ...
          const distanceRatio =
            Math.max(distToOrigin, distToDest) /
            Math.min(distToOrigin, distToDest);
