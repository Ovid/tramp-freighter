  ...
  it('should preserve all game state through save/load cycle', () => {
    fc.assert(
      fc.property(gameStateArbitrary, (generatedState) => {
        ...
        expect(loadedState.ship.cargoCapacity).toBe(
        ...
        for (let i = 0; i < generatedState.ship.cargo.length; i++) {
          expect(loadedState.ship.cargo[i].good).toBe(
            generatedState.ship.cargo[i].good
          );
        ...
        expect(loadedState.world.visitedSystems).toEqual(
      ...
      fc.property(
        ...
        (initialState, numCycles) => {
          ...
          for (let i = 0; i < numCycles; i++) {
            ...
            expect(loadedState.player.credits).toBe(
              currentState.player.credits
            );
            ...
            expect(loadedState.ship.cargo.length).toBe(
              currentState.ship.cargo.length
            );
