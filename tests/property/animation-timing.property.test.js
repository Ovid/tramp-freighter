...
describe('AnimationTimingCalculator - Property Tests', () => {
  ...
  it('Property 6: For any jump distance, travel duration should scale linearly with distance but be clamped to [1, 3] seconds', () => {
    ...
    const distanceGenerator = fc.oneof(
      ...
      fc.constantFrom(
        -5,
        0,
        ANIMATION_CONFIG.MIN_DISTANCE,
        ANIMATION_CONFIG.MAX_DISTANCE,
        30
      ),
    ...
    fc.assert(
      fc.property(distanceGenerator, (distance) => {
        ...
        if (
          ...
          distance <= ANIMATION_CONFIG.MAX_DISTANCE
        ) {
        ...
        if (distance >= 0 && distance < ANIMATION_CONFIG.MAX_DISTANCE) {
          const longerDistance = Math.min(
            distance + 1,
            ANIMATION_CONFIG.MAX_DISTANCE
          );
          const longerDuration =
            AnimationTimingCalculator.calculateTravelDuration(longerDistance);
  ...
  it('Property 7: For any jump, total animation duration should be between 3 and 5 seconds', () => {
    ...
    fc.assert(
      fc.property(distanceGenerator, (distance) => {
        const totalDuration =
          AnimationTimingCalculator.calculateTotalDuration(distance);
        ...
        const minTotalDuration =
          2 * ANIMATION_CONFIG.ZOOM_DURATION +
          ANIMATION_CONFIG.MIN_TRAVEL_DURATION;
