        ...
        fc.assert(
            fc.property(
                ...
                (initialCredits, newCredits) => {
                    ...
                    expect(document.getElementById('hud-credits').textContent).toBe(initialCredits.toLocaleString());
        ...
        fc.assert(
            fc.property(
                ...
                (initialDebt, newDebt) => {
                    ...
                    // Verify initial display
                    expect(document.getElementById('hud-debt').textContent).toBe(initialDebt.toLocaleString());
        ...
        fc.assert(
            fc.property(
                ...
                (initialFuel, newFuel) => {
                    ...
                    // Verify initial display
                    expect(document.getElementById('fuel-bar').style.width).toBe(`${initialFuel}%`);
                    expect(document.getElementById('hud-fuel-text').textContent).toBe(`${Math.round(initialFuel)}%`);
        ...
        fc.assert(
            fc.property(
                ...
                (initialCargo, newCargo) => {
                    ...
                    const initialCargoUsed = initialCargo.reduce((total, stack) => total + stack.qty, 0);
                    
                    // Verify initial display
                    expect(document.getElementById('hud-cargo').textContent).toBe(`${initialCargoUsed}/${cargoCapacity}`);
        ...
        fc.assert(
            fc.property(
                ...
                (initialSystemId, newSystemId) => {
                    ...
                    // Get initial system
                    const initialSystem = TEST_STAR_DATA.find(s => s.id === initialSystemId);
                    
                    // Verify initial display
                    expect(document.getElementById('hud-system').textContent).toBe(initialSystem.name);
                    ...
                    const distanceText = document.getElementById('hud-distance').textContent;
        ...
        fc.assert(
            fc.property(
                ...
                (initialDays, newDays) => {
                    ...
                    // Verify initial display
                    expect(document.getElementById('hud-days').textContent).toBe(String(initialDays));
        ...
        fc.assert(
            fc.property(
                ...
                (stateChanges) => {
                    ...
                    expect(document.getElementById('hud-credits').textContent).toBe(finalState.credits.toLocaleString());
                    expect(document.getElementById('hud-debt').textContent).toBe(finalState.debt.toLocaleString());
                    expect(document.getElementById('fuel-bar').style.width).toBe(`${finalState.fuel}%`);
                    expect(document.getElementById('hud-fuel-text').textContent).toBe(`${Math.round(finalState.fuel)}%`);
                    expect(document.getElementById('hud-days').textContent).toBe(String(finalState.days));
                }
            ),
            { numRuns: 100 }
        );
            ...
            fc.property(
                ...
                (goodType, quantity, price) => {
                    ...
                    if (result.success) {
                        ...
                        expect(document.getElementById('hud-credits').textContent).toBe(expectedCredits.toLocaleString());
                        
                        // Verify HUD updated cargo
                        const expectedCargoUsed = initialCargoUsed + quantity;
                        const cargoCapacity = gameStateManager.getShip().cargoCapacity;
                        expect(document.getElementById('hud-cargo').textContent).toBe(`${expectedCargoUsed}/${cargoCapacity}`);
                    }
                }
            ),
            { numRuns: 100 }
            ...
            fc.property(
                ...
                (amount) => {
                    ...
                    if (result.success) {
                        ...
                        expect(document.getElementById('fuel-bar').style.width).toBe(`${expectedFuel}%`);
                        expect(document.getElementById('hud-fuel-text').textContent).toBe(`${Math.round(expectedFuel)}%`);
                    }
                }
            ),
            { numRuns: 100 }
