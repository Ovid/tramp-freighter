    ...
    fc.assert(
      fc.property(
        ...
        (gameState, starData, systemIndex) => {
          ...
          expect(gameState.world.priceKnowledge[targetSystemId].lastVisit).toBe(
            0
          );
          expect(
            gameState.world.priceKnowledge[targetSystemId].prices
          ).toBeDefined();

          // All commodity prices should be present
          for (const goodType of Object.keys(BASE_PRICES)) {
            expect(
              gameState.world.priceKnowledge[targetSystemId].prices[goodType]
            ).toBeDefined();
            expect(
              typeof gameState.world.priceKnowledge[targetSystemId].prices[
                goodType
              ]
            ).toBe('number');
          }
    ...
    fc.assert(
      fc.property(
        ...
        (gameState, starData) => {
          ...
          const systemsWithKnowledge = Object.keys(
            gameState.world.priceKnowledge
          ).map(Number);
          ...
          if (result.success) {
            ...
            expect(
              gameState.world.priceKnowledge[targetSystemId].lastVisit
            ).toBe(0);
          }
        }
      ),
