    ...
    fc.assert(
      fc.property(
        ...
        (goodType, spectralClass) => {
          ...
          const calculatedPrice = TradingSystem.calculatePrice(
            goodType,
            spectralClass
          );
          ...
          const modifier =
            SPECTRAL_MODIFIERS[spectralLetter]?.[goodType] || 1.0;
          ...
          expect(calculatedPrice).toBeGreaterThanOrEqual(
            Math.floor(basePrice * 0.5)
          );
          expect(calculatedPrice).toBeLessThanOrEqual(
            Math.ceil(basePrice * 1.6)
          );
        }
      ),
      { numRuns: 100 }
