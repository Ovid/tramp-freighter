...
describe('BaseManager log/warn gap coverage', () => {
  ...
  describe('log()', () => {
    ...
    it('calls console.log with subclass name when DEV_MODE is true', () => {
      ...
      const manager = new ShipManager(makeCapabilities());
  ...
  describe('warn()', () => {
    ...
    it('calls console.warn with subclass name when DEV_MODE is true', () => {
      ...
      const manager = new TradingManager(makeCapabilities());
  ...
  // ── validateState() is a no-op in capability mode ──

  describe('validateState() in capability mode', () => {
    it('does not throw (no-op)', () => {
      class DangerManager extends BaseManager {}
      const manager = new DangerManager(makeCapabilities());
      expect(() => manager.validateState()).not.toThrow();
    });
  });

  // ── Capabilities stored through subclass ──

  describe('capability storage through subclass', () => {
    class SubManager extends BaseManager {}

    it('stores capabilities object', () => {
      const caps = makeCapabilities();
      const manager = new SubManager(caps);
      expect(manager.capabilities).toBe(caps);
    });

    it('stores isTestEnvironment flag', () => {
      const manager = new SubManager(
        makeCapabilities({ isTestEnvironment: true })
      );
      expect(manager.isTestEnvironment).toBe(true);
    });

    it('defaults isTestEnvironment to false', () => {
      const manager = new SubManager({});
