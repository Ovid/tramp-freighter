...
describe('NPCManager coverage', () => {
  ...
  describe('canGetFreeRepair', () => {
    it('returns unavailable for low rep', () => {
      gsm.npcManager.getNPCState('chen_barnards').rep = 30;
      const result = gsm.npcManager.canGetFreeRepair('chen_barnards');
      expect(result.available).toBe(false);
      expect(result.reason).toContain('Requires Trusted');
    });

    it('returns available for Trusted tier', () => {
      gsm.npcManager.getNPCState('chen_barnards').rep = REPUTATION_BOUNDS.TRUSTED_MIN;
      const result = gsm.npcManager.canGetFreeRepair('chen_barnards');
      expect(result.available).toBe(true);
    ...
    it('returns higher repair limit for Family tier', () => {
      gsm.npcManager.getNPCState('chen_barnards').rep = REPUTATION_BOUNDS.FAMILY_MIN;
      const result = gsm.npcManager.canGetFreeRepair('chen_barnards');
      expect(result.available).toBe(true);
      expect(result.maxHullPercent).toBe(NPC_BENEFITS_CONFIG.FREE_REPAIR_LIMITS.family);
    });

    it('returns unavailable when already used today', () => {
      const npcState = gsm.npcManager.getNPCState('chen_barnards');
      npcState.rep = REPUTATION_BOUNDS.TRUSTED_MIN;
      npcState.lastFreeRepairDay = gsm.state.player.daysElapsed;
      const result = gsm.npcManager.canGetFreeRepair('chen_barnards');
      expect(result.available).toBe(false);
  ...
  describe('getFreeRepair', () => {
    it('fails when repair unavailable', () => {
      gsm.npcManager.getNPCState('chen_barnards').rep = 0;
      const result = gsm.npcManager.getFreeRepair('chen_barnards', 50);
      expect(result.success).toBe(false);
    });

    it('fails for invalid hull damage percent', () => {
      gsm.npcManager.getNPCState('chen_barnards').rep = REPUTATION_BOUNDS.TRUSTED_MIN;
      const result = gsm.npcManager.getFreeRepair('chen_barnards', -10);
      expect(result.success).toBe(false);
      expect(result.message).toBe('Invalid hull damage percentage');
    });

    it('returns zero repair when no damage', () => {
      gsm.npcManager.getNPCState('chen_barnards').rep = REPUTATION_BOUNDS.TRUSTED_MIN;
      const result = gsm.npcManager.getFreeRepair('chen_barnards', 0);
      expect(result.success).toBe(true);
      expect(result.repairedPercent).toBe(0);
    });

    it('repairs up to max hull percent', () => {
      gsm.npcManager.getNPCState('chen_barnards').rep = REPUTATION_BOUNDS.TRUSTED_MIN;
      gsm.state.ship.hull = 50;
      const result = gsm.npcManager.getFreeRepair('chen_barnards', 50);
      expect(result.success).toBe(true);
      expect(result.repairedPercent).toBe(NPC_BENEFITS_CONFIG.FREE_REPAIR_LIMITS.trusted);
    });

    it('sets lastFreeRepairDay', () => {
      gsm.npcManager.getNPCState('chen_barnards').rep = REPUTATION_BOUNDS.TRUSTED_MIN;
      gsm.state.ship.hull = 50;
      gsm.npcManager.getFreeRepair('chen_barnards', 50);
      expect(gsm.npcManager.getNPCState('chen_barnards').lastFreeRepairDay).toBe(
        gsm.state.player.daysElapsed
      );
    });
  });
