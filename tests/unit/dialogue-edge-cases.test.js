...
describe('Dialogue Engine Edge Cases', () => {
  ...
  describe('Dialogue ending when choice has no next node', () => {
    it('should return null when selecting a choice with next: null', () => {
      ...
      const endingChoice = initialDialogue.choices.find(choice => choice.next === null);
    ...
    it('should return null for all NPCs when selecting ending choices', () => {
      ...
      for (const npcId of npcIds) {
        ...
        // Find ending choices
        const endingChoices = initialDialogue.choices.filter(choice => choice.next === null);
  ...
  describe('Greeting node is displayed by default', () => {
    it('should show greeting node when no specific node is requested', () => {
      ...
      expect(dialogue.text).toBe('Another trader. Docking fees paid? Good. What you need?');
  ...
  describe('Uninitialized NPC uses initialRep for dialogue display', () => {
    ...
    it('should use Marcus Cole initialRep (-20) for uninitialized NPC', () => {
      ...
      expect(dialogue.text).toBe('Your debt remains outstanding. I trust you have good news for me.');
    ...
    it('should use Father Okonkwo initialRep (10) for uninitialized NPC', () => {
      ...
      expect(dialogue.text).toBe("Welcome back, child. It does my heart good to see you safe. The void between stars can be lonely - you're always welcome here.");
    ...
    it('should transition from initialRep to actual rep after first interaction', () => {
      ...
      expect(updatedDialogue.text).toBe('Hey there, friend! Good to see you again. Ship treating you well?');
    });
  });
  ...
  describe('Error handling', () => {
    ...
    it('should throw error for unknown dialogue node', () => {
      ...
      }).toThrow('Unknown dialogue node: nonexistent_node for NPC: chen_barnards');
    });
    ...
    it('should throw error for invalid choice index', () => {
      ...
      }).toThrow('Invalid choice index: 999 for node greeting in NPC chen_barnards');
      
      expect(() => {
        selectChoice(npcId, -1, gameStateManager);
      }).toThrow('Invalid choice index: -1 for node greeting in NPC chen_barnards');
    });
  });
  ...
  describe('Dialogue state consistency', () => {
    ...
    it('should handle reputation changes during dialogue correctly', () => {
      ...
      const repGainChoice = nextDialogue.choices.find(c => c.repGain && c.repGain > 0);
      if (repGainChoice) {
        const finalDialogue = selectChoice(npcId, repGainChoice.index, gameStateManager);
