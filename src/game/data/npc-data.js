...
export function validateNPCDefinition(npc) {
  ...
  for (const field of requiredFields) {
    if (!(field in npc)) {
      throw new Error(`Invalid NPC definition: missing required field '${field}'`);
    }
  }
  ...
  for (const trait of requiredPersonalityTraits) {
    if (!(trait in npc.personality)) {
      throw new Error(`Invalid NPC definition: missing personality trait '${trait}'`);
    }
  }
  ...
  for (const prop of requiredSpeechProperties) {
    if (!(prop in npc.speechStyle)) {
      throw new Error(`Invalid NPC definition: missing speechStyle property '${prop}'`);
    }
  }
}
...
export const WEI_CHEN = {
  ...
  description: 'A weathered dock worker with calloused hands and knowing eyes. Former ship captain.',
...
export const MARCUS_COLE = {
  ...
  description: 'Impeccably dressed financier with cold eyes and a calculating smile. Your creditor.',
...
export const FATHER_OKONKWO = {
  ...
  description: 'Gentle chaplain with kind eyes and a warm smile. Offers comfort to weary travelers.',
