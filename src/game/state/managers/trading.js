...
export class TradingManager extends BaseManager {
  ...
  buyGood(goodType, quantity, price) {
    ...
    const cargoSpace = this.capabilities.getCargoRemaining();
  ...
  sellGood(stackIndex, quantity, salePrice) {
    ...
    const { withheld } = this.capabilities.applyTradeWithholding(totalRevenue);
    const playerReceives = totalRevenue - withheld;
    this.capabilities.updateCredits(
      this.capabilities.getCredits() + playerReceives
    );

    this.capabilities.updateStats('cargoHauled', quantity);
    this.capabilities.updateStats('creditsEarned', totalRevenue);
    ...
    this.capabilities.updateCargo([...cargo]);
  ...
  getCurrentSystemPrices() {
    ...
    if (!ownState.currentSystemPrices) {
      throw new Error(
        'Invalid state: currentSystemPrices missing from world state'
      );
    }
    return ownState.currentSystemPrices;
  }
  ...
  updateMarketConditions(systemId, goodType, quantityDelta) {
    ...
    if (!ownState.marketConditions[systemId]) {
      ownState.marketConditions[systemId] = {};
    }

    // Create commodity entry if first trade of that commodity
    if (ownState.marketConditions[systemId][goodType] === undefined) {
      ownState.marketConditions[systemId][goodType] = 0;
    }

    // Add quantityDelta to existing value
    ownState.marketConditions[systemId][goodType] += quantityDelta;
  }
  ...
  applyMarketRecovery(daysPassed) {
    ...
    for (const systemId in ownState.marketConditions) {
      const systemConditions = ownState.marketConditions[systemId];
      ...
      if (Object.keys(systemConditions).length === 0) {
        delete ownState.marketConditions[systemId];
      }
    }
  }
  ...
  recordVisitedPrices() {
    ...
    this.updatePriceKnowledge(currentSystemId, currentPrices, 0, INTELLIGENCE_CONFIG.SOURCES.VISITED);
  ...
  incrementPriceKnowledgeStaleness(days = 1) {
    const ownState = this.capabilities.getOwnState();
    if (!ownState.priceKnowledge) {
      throw new Error('Invalid state: priceKnowledge missing from world state');
    }

    for (const systemId in ownState.priceKnowledge) {
      ownState.priceKnowledge[systemId].lastVisit += days;
    }
  ...
  recalculatePricesForKnownSystems() {
    const ownState = this.capabilities.getOwnState();
    if (!ownState.priceKnowledge) return;
    ...
    const marketConditions = ownState.marketConditions;
    if (!marketConditions) {
      throw new Error(
        'Invalid state: marketConditions missing from world state'
      );
    ...
    for (const systemIdStr in ownState.priceKnowledge) {
      ...
      if (system && ownState.priceKnowledge[systemId].prices !== null) {
        ...
        ownState.priceKnowledge[systemId].prices = newPrices;
      }
    }
  ...
  getPriceKnowledge() {
    const ownState = this.capabilities.getOwnState();
    return ownState.priceKnowledge || {};
  }
  ...
  getKnownPrices(systemId) {
    ...
    return ownState.priceKnowledge[systemId]?.prices || null;
  }
  ...
  hasVisitedSystem(systemId) {
    ...
    return ownState.priceKnowledge[systemId] !== undefined;
  }
  ...
  isGoodRestricted(goodType, systemId) {
    ...
    const dangerZone = this.capabilities.getDangerZone(systemId);
