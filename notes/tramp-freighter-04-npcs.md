...
const NPC_SCHEMA = {
  ...
  description: "Weathered dock worker in her 50s. Seen it all, helps those who help themselves.",
...
const cole_sol = {
  ...
  description: "Your creditor. Starts hostile but respects those who pay honestly.",
  ...
  chen_barnards: {
    greeting: {
      ...
      choices: [
        ...
        { text: "Tell me about yourself.", next: "backstory", condition: "rep >= 30" },
...
function getServicePrice(basePrice, npcId) {
  ...
  if (tier.name === "Friendly") return Math.round(basePrice * 0.90);  // 10% off
  if (tier.name === "Trusted") return Math.round(basePrice * 0.85);  // 15% off
  if (tier.name === "Family") return Math.round(basePrice * 0.80);  // 20% off
