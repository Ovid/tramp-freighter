Product Requirements Document: Sol Sector Starmap Visualization
Product Owner: Gemini
Date: December 3, 2025
Status: Ready for Development (V1.1)
Feature: Dynamic, Interactive 3D Star Map (Sol Vicinity)

1. Goal & Objectives
   The primary goal of this feature is to transform static astronomical data into an engaging, dynamic, and intuitive three-dimensional user experience. This visualization serves as the primary navigation and exploration tool for the star systems within 20 light-years of Sol.
   Key Objectives:
   Enhance Depth Perception: Provide clear visual cues to distinguish between nearby and distant star systems using dynamic scaling and opacity.
   Improve Usability: Ensure easy, responsive navigation across various input methods (mouse, touch).
   Data Clarity: Clearly present system data (classification, wormholes, status) upon interaction.
   Sci-Fi Aesthetic: Deliver a professional, immersive, and futuristic visual experience with neon accents and volumetric effects.
2. Target Users
   Explorers/Researchers: Users needing quick, spatial comprehension of the local interstellar network.
   Game/Scenario Planners: Users who utilize connectivity and system status for strategic planning.
3. Scope & Requirements
   3.1 Data Requirements
   Source Data: Must integrate data for all known star systems within 20 light-years of Sol. The exact data set to be used is listed below.
   Data Structure: id (integer), x, y, z (coordinates in light years \* 10 or similar scale), name (string), type (spectral class string), wh (wormhole count), st (station count - inferred), r (reachable boolean: 1=true, 0=false).
   Complete Star Data Set (JSON format for implementation):
   [
...
Wormhole Connectivity Data (Array of ID pairs):
[
...
Status
FR.1
3D Navigation: Users must be able to pan (RMB), rotate/orbit (LMB), and zoom/dolly (Scroll Wheel) through the 3D space.
High
Complete
FR.2
On-Screen Controls: Provide touch/click-friendly buttons for "Zoom In," "Zoom Out," and "Toggle Rotation" to supplement mouse input. Zoom sensitivity must be set to 150 for responsiveness.
High
Complete
FR.3
System Selection: The user must be able to select a star either by clicking its visual sprite or its visible label.
High
Complete
FR.4
Information Panel: Upon star selection (FR.3), a detailed HUD panel must display the system's Name, Coordinates, Spectral Class, Connection Count, and Reachability Status.
High
Complete

3.3 Visual & Experience Requirements
ID
Requirement
Priority
Status
UX.1
Visual Theme: The map must utilize a dark, neon sci-fi aesthetic, including a subtle volumetric background glow and pulsing star sprites.
High
Complete
UX.2
Wormhole Visibility: Connections must be highly visible and color-coded based on status (e.g., bright blue for Stable/Reachable; dull red for Unstable/Unreachable).
High
Complete
UX.3
Depth-of-Field (Labels): Star system labels must dynamically adjust their size and opacity based on their distance from the camera. Closer labels (MAX_FONT_SIZE: 18px) should be larger and fully opaque, while distant labels (MIN_OPACITY: 0.1) should fade almost completely.
High
Complete
UX.4
Initial View: The map must load starting at a fully zoomed-out perspective (Camera Position: 500, 500, 500), centered on Sol, ensuring the entire sector is visible.
High
Complete
UX.5
Sector Boundary: Implement a toggleable, large wireframe sphere (Radius: 300) showing clean latitude and longitude lines (using EdgesGeometry, NOT triangles) to delineate the extent of the exploration sector. This grid must start visible upon load.
High
Complete
UX.6
Control Contrast: All on-screen button labels must remain clearly legible (black text) when the button is in an active (bright green background) state.
High
Complete

4. Technical Specifications (Abstraction Layer)
   The visualization layer for this feature utilizes the Three.js framework to render the interactive 3D environment. This architecture allows for hardware-accelerated rendering, complex material handling for visual effects (like glow and volumetric fog), and flexible control of the virtual camera.
5. Success Metrics
   Usability: 95% of users successfully navigate and find a target star system within 30 seconds.
   Performance: Map loads in under 3 seconds.
   Adoption: The 3D map is used as the primary data visualization tool over legacy table views.
