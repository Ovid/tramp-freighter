...
- Create a `GameContext` that holds the _instance_ of `GameStateManager`.
...
- **Mechanism:**
  ...
  3. When the event fires, the hook updates a local `useState`, triggering a re-render of _only_ that component.
...
- **Behavior:**
  ...
  - **Crucial:** Does _not_ re-run `initScene` on React updates.
