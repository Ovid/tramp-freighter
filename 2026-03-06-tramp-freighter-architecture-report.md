...
9. **[Impact: Low] Three.js selection state outside GameStateManager** — Star selection in the 3D view (`selectedStar`, `currentSystemIndicator`) is tracked in module-level variables in `interaction.js`, not through GameStateManager. This is UI-only state but creates an asymmetry.
   Evidence: `src/game/engine/interaction.js:4-7` (module-level `let selectedStar = null`)

10. **[Impact: Low] GameStateManager facade lacks organizational markers** — 177 delegation methods in a 1,208-line file with no section comments grouping methods by their target manager. Navigation relies on search rather than visual scanning.
    Evidence: `src/game/state/game-state-manager.js` (1,208 lines, 177 methods, no section headers)
...
1. **`src/game/state/game-state-manager.js` (1,208 lines)** — Central facade with 177 methods. Well-designed as delegation hub, but its size makes navigation difficult. Contains the encapsulation violation pathway to NPCManager → ShipManager. Review for organizational improvements.
