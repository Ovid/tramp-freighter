         ...
         * Note: This is embedded in HTML for now. The standalone game-state.js module
         * is used for testing and will be used when serving via HTTP server.
         */
        ...
        function updateHUD(star) {
            if (!star || !star.data) {
                console.warn('Cannot update HUD: invalid star data');
                return;
            }
            ...
            if (hudName) {
                hudName.textContent = star.data.name;
            }
            
            // Update coordinates (x, y, z)
            const hudCoords = document.getElementById('hud-coords');
            if (hudCoords) {
                hudCoords.textContent = `${star.data.x}, ${star.data.y}, ${star.data.z}`;
            }
            
            // Update spectral class
            const hudSpectral = document.getElementById('hud-spectral');
            if (hudSpectral) {
                hudSpectral.textContent = star.data.type;
            }
            
            // Update wormhole connection count
            const hudWormholes = document.getElementById('hud-wormholes');
            if (hudWormholes) {
                hudWormholes.textContent = star.data.wh;
            }
            
            // Update reachability status (interpret r: 1="Reachable", 0="Unreachable")
            const hudReachable = document.getElementById('hud-reachable');
            if (hudReachable) {
                hudReachable.textContent = star.data.r === 1 ? 'Reachable' : 'Unreachable';
            }
            
            console.log(`HUD updated with data for ${star.data.name}`);
        }
        ...
        function updateAutoRotation() {
            if (autoRotationEnabled && controls) {
                // Rotate camera around the scene center at 0.2 degrees per frame
                // Convert degrees to radians: 0.2 degrees = 0.2 * (Math.PI / 180)
