...
export class EconomicEventsSystem {
     ...
     */
    static COMMODITY_TYPES = ['grain', 'ore', 'tritium', 'parts', 'medicine', 'electronics'];
    ...
    static updateEvents(gameState, starData) {
        ...
        activeEvents = EconomicEventsSystem.removeExpiredEvents(activeEvents, currentDay);
    ...
    static isSystemEligible(system, eventType) {
        ...
        if (targetSystems === "mining") {
            ...
            return EconomicEventsSystem.MINING_SPECTRAL_CLASSES.includes(spectralLetter);
        }
    ...
    static createEvent(eventTypeKey, systemId, currentDay) {
        ...
        const duration = minDuration + Math.floor(rng.next() * (maxDuration - minDuration + 1));
        ...
        if (eventTypeKey === 'supply_glut') {
            ...
            const commodityIndex = Math.floor(commodityRng.next() * EconomicEventsSystem.COMMODITY_TYPES.length);
            const randomCommodity = EconomicEventsSystem.COMMODITY_TYPES[commodityIndex];
